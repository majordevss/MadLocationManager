...
public class MainActivity extends AppCompatActivity implements LocationServiceInterface, MapInterface, ILogger {
    ...
    public void locationChanged(Location location) {
        if (m_map != null && m_presenter != null) {
            CameraPosition cameraPosition = new CameraPosition.Builder().tilt(0).target(new LatLng(location.getLatitude(), location.getLongitude())).zoom(m_map.getMaxZoomLevel() - 4).build();
            m_map.moveCamera(CameraUpdateFactory.newCameraPosition(cameraPosition));
            m_presenter.locationChanged(location, cameraPosition);
        }
    }
    ...
    private int routeColors[] = {R.color.orange, R.color.colorAccent, R.color.green};
    ...
    public void setupMap(@Nullable Bundle savedInstanceState) {
        ...
        m_mapView.getMapAsync(mapboxMap -> {
           ...
           // m_map.setMapStyle(BuildConfig.lightMapStyle);
            ...
            m_map.setPadding(leftMargin, topMargin, rightMargin, bottomMargin);
